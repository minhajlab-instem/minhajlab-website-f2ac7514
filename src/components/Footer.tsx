...
const Footer: React.FC = () => {
  ...
  const resourceLinks = [
    { name: 'For Students', path: '#' }, // Placeholder
    { name: 'Laboratory Protocols', path: '#' }, // Placeholder
    { name: 'Data & Code', path: '#' }, // Placeholder
    { name: 'Funding Sources', path: '#' }, // Placeholder
  ];

  const connectLinks = [
    { name: 'Contact Us', path: '/contact' },
    // Using available icons, will add placeholders for others if not available
    { name: 'Twitter', path: '#', icon: <Twitter size={20} /> }, // Placeholder URL
    { name: 'LinkedIn', path: '#', icon: <Linkedin size={20} /> }, // Placeholder URL
    { name: 'Google Scholar', path: '#' }, // Placeholder, no direct icon
  ];

  return (
    <footer className="bg-slate-800 text-slate-300 py-12 mt-auto font-sans">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          {/* Lab Info */}
          <div className="col-span-1 md:col-span-1">
            <h2 className="text-xl font-semibold text-white mb-2">Cytoskeleton Research Lab</h2>
            ...
            <ul className="space-y-2">
              {connectLinks.map((link) => (
                <li key={link.name}>
                  <a
                    href={link.path}
                    target={link.path.startsWith('http') || link.path === '#' ? '_blank' : '_self'}
